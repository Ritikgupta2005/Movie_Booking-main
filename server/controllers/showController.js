  ...
  try {
    ...
    if (!movieId || !showPrice || !Array.isArray(showsInput)) {
      return res
        ...
        .json({
          success: false,
          message: "movieId, showPrice, and showsInput[] are required.",
        });
    }
    ...
    if (!movie) {
      const [detailsRes, creditRes] = await Promise.all([
        axios.get(`https://api.themoviedb.org/3/movie/11/${movieId}`, {
          headers: { Authorization: `Bearer ${process.env.TMDB_API_KEY}` },
        }),
        axios.get(`https://api.themoviedb.org/3/movie/11/${movieId}/credits`, {
          headers: { Authorization: `Bearer ${process.env.TMDB_API_KEY}` },
        }),
      ]);
